...
<template>
  <button 
    ...
    class="inline-flex items-center justify-center h-8 px-3 bg-white dark:bg-gray-700 rounded-md text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
    :title="themeStore.isDarkMode ? languageStore.t('switchToLightMode') : languageStore.t('switchToDarkMode')"
  >
    <!-- Sun icon for dark mode (to switch to light) -->
    <svg v-if="themeStore.isDarkMode" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5">
